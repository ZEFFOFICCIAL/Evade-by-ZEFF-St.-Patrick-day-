...
CreditLabel.Parent, CreditLabel.Size, CreditLabel.Position = Frame, UDim2.new(0, 200, 0, 30), UDim2.new(0, 0, 0, -35)
CreditLabel.BackgroundTransparency, CreditLabel.Text, CreditLabel.Font, CreditLabel.TextSize = 1, "By ZEFF", LogoFont, 28 

VersionLabel.Parent, VersionLabel.Size, VersionLabel.Position = Frame, UDim2.new(0, 40, 0, 20), UDim2.new(1, -45, 1, -22)
...
local UIS, Lighting, RunService = game:GetService("UserInputService"), game:GetService("Lighting"), game:GetService("RunService")
    ...
    if root and hum and hum.Health > 0 then
        ...
        if flying then
            ...
            if hum:GetState() == Enum.HumanoidStateType.Crouching or hum:GetState() == Enum.HumanoidStateType.Physics then
                ...
                root.AssemblyLinearVelocity = Vector3.new(hum.MoveDirection.X * (sMult * 50), root.AssemblyLinearVelocity.Y, hum.MoveDirection.Z * (sMult * 50))
            end
        end
    end
end)
...
UIS.InputBegan:Connect(function(i, g)
    ...
    if i.KeyCode == Enum.KeyCode.N then noclip = not noclip if not noclip then 
        local char = game.Players.LocalPlayer.Character
    ...
    if i.KeyCode == Enum.KeyCode.B then 
        ...
        if bright_enabled then 
            Lighting.Brightness = 2; Lighting.ClockTime = 14; Lighting.OutdoorAmbient = Color3.new(1,1,1); Lighting.FogEnd = 1e5
        else Lighting.Brightness = 1; Lighting.ClockTime = 0; Lighting.OutdoorAmbient = Color3.new(0,0,0) end
    end
end)
